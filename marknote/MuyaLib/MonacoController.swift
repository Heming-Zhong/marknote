...
// MARK: - MonacoController Coordinator

public class MonacoController: NSObject {
    
    // MARK: Properties
    
    var parent: MonacoView
    weak var webView: WKWebView?
    
    fileprivate var pageLoaded = false
    fileprivate var pendingFunctions = [JavascriptFunction]()
    
    init(_ parent: MonacoView) {
        self.parent = parent
    }
...
extension MonacoController: WKScriptMessageHandler {
...
extension MonacoController: WKNavigationDelegate {
    public func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        
        // is Ready
        if message.name == MonacoViewRPC.isReady {
            pageLoaded = true
